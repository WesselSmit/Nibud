...
function determineYourSituation() { //create personal houseHold Object with uw_sitautie data/input
    ...
    if (yourSituation.partner == "true" && yourSituation.kinderen == 3 || yourSituation.partner == "true" && yourSituation.kinderen == 4 || yourSituation.partner == "true" && yourSituation.kinderen == 5) {
...
document.addEventListener('scroll', function () { //hide demo button on scroll in barchart viewport + fix scroll indicators visibilities
    ...
    if (secondSection.classList.contains('hide') === false &&
        ...
        if (secondSection.classList.contains('hide') === true) {
            document.getElementById('moneyIndicator').classList.add('hide')
        } else {
...
function getHighestExpenses() {
    ...
    for (let i = 0; i < bespaarTipsArray.length; i++) {
        bespaarDivs[i].querySelector('img').src = "./media/sims/" + bespaarTipsArray[i].post.replace(" ", "_").replace(" ", "_") + ".svg"
